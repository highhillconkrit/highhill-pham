...
export default function Calendar({ onSwitchView }: { onSwitchView?: (view: "list" | "calendar") => void }) {
  ...
  const groupBookingsByDay = (list: BookingRecord[], targetYear: number, targetMonth: number) => {
  ...
  const bookingsByDay = (targetYear: number, targetMonth: number) =>
    groupBookingsByDay(bookings, targetYear, targetMonth);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { year: sy, month: sm, day: sd } = selectedDate;
    const targetEmail = form.email.trim().toLowerCase();

    const results = await loadBookings();
    const freshBookings = results.bookings;
    const freshMap = groupBookingsByDay(freshBookings, sy, sm);

    const dayBookings = freshMap[sd] ?? [];
    const targetExists = dayBookings.some(b => b.slot === selectedSlot);

    if (targetExists) {
      const own = dayBookings.find(b => b.slot === selectedSlot && b.email.toLowerCase() === targetEmail);
      if (own) {
        alert("A booking with this email already exists on this date for this slot.");
      } else {
        alert("This slot has already been booked. Please refresh and choose another.");
      }
      return;
    }

    const weekend = isWeekend(sy, sm, sd);
    const slotConfig = results.operatingHours.find(h =>
      h.dayType === (weekend ? "weekend" : "weekday") && h.slot === selectedSlot
    );
    if (slotConfig && !slotConfig.enabled) {
      alert(`${selectedSlot === "morning" ? "The AM" : selectedSlot === "evening" ? "The PM" : "This"} slot is currently closed for bookings.`);
      return;
    }

    const monthCount = Object.values(freshMap).flat().filter(b => b.email.toLowerCase() === targetEmail).length;
    if (monthCount >= 7) {
